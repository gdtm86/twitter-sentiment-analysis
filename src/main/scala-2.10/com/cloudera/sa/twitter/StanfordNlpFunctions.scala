...
/**
 * Created by gmedasani on 12/19/16.
 */
...
object StanfordNlpFunctions {
   ...
   * positive).
   * If the input contains multiple sentences, sentiments for all the sentences are calculated.
   * @see [[Sentiment]]
   */
  def sentencesentiment = udf { sentences: Seq[String] =>
    val pipeline = getOrCreateSentimentPipeline()
    val sentiments = ListBuffer[Int]()
    for (sentence:String <- sentences){
      val annotation = pipeline.process(sentence.toString)
      ...
      sentiments += RNNCoreAnnotations.getPredictedClass(tree)
    }
    sentiments.toList.toArray
  }

  /**
   * Measures the sentiment of an input document on a scale of 0 (strong negative) to 4 (strong
   * positive).
   * If the input contains multiple sentences, only the first one is used. (Other heuristics can be tried as well)
   * @see [[Sentiment]]
   */
  def documentsentiment = udf { sentence: String =>
    val pipeline = getOrCreateSentimentPipeline()
  ...
  /**
   * Splits a document into sentences and calculates number of sentences.
   * @see [[Document#sentences]]
   */
  def ssplitlen = udf { document: String =>
    val splitSentences = new Document(document).sentences().asScala.map(_.text())
